...
public class Spielverwaltung implements NotifyCallback {
	 ...
	 * Spieler-Liste Aktualisiert, für den Fall, dass neue Nodes in der FingerTabler
	 ...
	 * unsere Liste, den wir beschießen wollen, wird ausgesucht 5. Es wird eine
	 * genau ID, die der Spieler aus 4. verwaltet, ausgesucht und auf sie geschoßen
	 * (Retrieve).
	 */
	@Override
	 ...
	 * Zuerst werden die Spieler-Liste Aktualisiert, für den Fall, dass neue Nodes
	 * in der FingerTabler aufgetaucht sind. 2. Der Absender des Broadcasts wird zu
	 * unsere Spieler-Liste hinzugefügt, falls noch nicht vorhanden und es sich
	 ...
	 * Spielers und der ID seines Vorgänger liegt oder ob es sich um die ID des
	 ...
	 * 
	 * @param target
	 *            ID auf die geschoßen wurde
	 * @param hit
	 *            Zeigt, ob ein Schiff getroffen wurde oder nicht
	 * @param spieler
	 *            Augewähler Spieler aus der Spieler-Liste
	 */
	 ...
	 * dieser Spieler zu unseren Spieler-Liste hinzugefügt und die vorgaenger IDs in
	...
	/**
	 * Nehme eine zufähliger Feld(index) aus den noch verfügbaren Felder des
	 * Ziel-Spielers, rechne diesen Index dann in die ID um und liefere sie zurück
	 * 
	 * @param zielSpieler
	 *            Auf ihn sollte geschoßen werden
	...
	/**
	 * Hollt sich die FingerTable und fügt neu dazugekommene Spieler in unsere
	 * Spieler-Liste hinzu, dabei werden die vorgänger IDs in den Spielern
	 * aktualiesiert
	 */
	...
	/**
	 * Aktualisiert Spieler-Liste. Schaut, ob wir die größtmöglich ID verwalten,
	 * wenn ja dürfen wir zuerst Schießen
	 */
	public void erstelleSpiel() {
		...
		if (meinSpieler.getPreviousPlayerID() != null
			...
			if (zielSpieler != null) {
				...
				System.out.println("Niemand da auf dem man schießen kann");
			}
		} else {
	...
	/**
	 * Erstellt unseren eingenen Spieler mit der ID von Chord und unseren vorgänger
	 * ID. Setzt auf die 100 Feldern 10 Schiffe auf zufählige Positionen/Feldern.
	 */
